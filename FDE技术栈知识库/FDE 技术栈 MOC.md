...
- [[FDE 交付 Playbook]] —— 从发现、试点到上线交接
- [[产品验证与用户访谈]] —— 从模糊想法提炼可验证假设
- [[PRD 与技术文档]] —— 把需求、技术方案和 AI 执行对齐
- [[用户反馈与产品迭代]] —— 上线后的反馈、指标与迭代闭环
...
- [[Node.js 项目环境与本地运行]] —— Node 版本、pnpm、项目启动和 localhost
...
- [[托管数据库与 Drizzle]] —— Neon / Supabase、Schema、迁移、RLS 与备份
...
- [[Agent]] —— 自主规划与执行的智能体
- [[MCP]] —— 模型上下文协议，工具标准化
- [[Claude Code 工作流与项目配置]] —— Agent Native、项目规则、Hooks 与成本控制
...
- [[系统集成 Runbook]] —— 契约、回放、验收与故障归属
- [[Web 用户认证与安全]] —— Session、RBAC、路由保护与 Web 安全
...
- [[Web 测试与 E2E]] —— API、Playwright、Flaky Test 与测试闭环
- [[SEO、分享与产品分析]] —— SEO、Open Graph、Umami 与基础合规
- [[数据契约与质量]] —— Schema、质量、血缘、回填与兼容性
...
每篇笔记的 frontmatter 都包含 `sources`、`reviewed` 和 `stability`。
`evergreen` 表示相对稳定的原理；`moving` 表示模型、API、云服务或产品能力可能快速变化的内容。
事实、经验和推论应在正文中明确区分，易变内容优先回到官方文档、标准或论文核验。
