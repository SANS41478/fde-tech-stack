...
所有后台任务、Webhook、缓存键、对象存储路径和导出任务都要携带租户上下文。不要接受客户端直接传入的 `tenant_id` 作为唯一依据，也不要使用没有租户前缀的共享缓存键。

## 九、补充威胁面

- SSRF：限制出站协议、域名、解析结果和重定向，禁止让用户输入直接决定内网请求。
- Webhook 重放：校验时间戳、事件 ID 和短期去重窗口。
- 依赖供应链：锁定版本、审计漏洞、审查安装脚本和构建产物。
- 审计事件：记录主体、租户、动作、对象、结果、来源、版本和 request_id，日志本身不可由业务 Agent 删除。
